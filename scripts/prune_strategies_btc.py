...
# Pruning criteria for BTCUSDm M15:
# - Remove obviously bad / useless strategies to reduce noise.
#
# A strategy is pruned if:
#   1) symbol == "BTCUSDm" and timeframe == "M15", AND
...
def prune_pool() -> None:
    ...
    for name, rec in pool.items():
        ...
        # Only prune BTCUSDm M15 here
        if symbol != "BTCUSDm" or timeframe != "M15":
            kept[name] = rec
            continue
        ...
        # Dummy strategies with no trades / no edge
        is_dummy = (num_trades == 0.0 and return_pct == 0.0 and not accepted)

        # Clearly weak / bad strategies
        is_weak = (
