...
- T4/T5 broker-validation hardening is now complete: `execution/manual_trade_identity.py` now assigns a dedicated manual MT5 magic number, `execution/manual_trade_broker_validation.py` builds broker-facing `order_check` requests with manual_user markers for both market and limit drafts, and `POST /api/execution/manual-ticket/preview-intent` now rejects broker-invalid drafts with operator-readable errors before audit persistence while returning broker-validation details on success
- `ui-front/src/app/manual-ticket/page.tsx` now surfaces the dedicated manual magic marker in the preview and shows a specific broker-rejection state when MT5 validation fails during confirmed preview recording
- Verified the broker-validation slice with `python -m pytest autonomous_trading_ai/tests/test_manual_trade_risk_api.py autonomous_trading_ai/tests/test_manual_trade_identity_ui_api.py`, `python -m compileall autonomous_trading_ai\ui_api autonomous_trading_ai\execution`, and a production `npm run build` in `ui-front`
- T6 backend live-submit scaffold is now complete: `ui_api/app.py` exposes `POST /api/execution/manual-ticket/submit`, `execution/manual_trade_submit.py` sends manual_user-tagged market or limit MT5 orders, submit now records both `manual_ticket_submit_intent` and `manual_ticket_execution_result`, and `client_submission_id` makes retries idempotent instead of duplicating orders
- The submit endpoint keeps the explicit operator gate server-side with `confirm_submit=true`, reruns broker validation before send, returns raw broker response details to the UI, and preserves the manual_user / exclude_from_strategy_eval markers across audit events
- Verified the T6 backend submit slice with `python -m pytest autonomous_trading_ai/tests/test_manual_trade_risk_api.py autonomous_trading_ai/tests/test_manual_trade_identity_ui_api.py autonomous_trading_ai/tests/test_strategy_live_stats_manual_buckets.py` and `python -m compileall autonomous_trading_ai\ui_api autonomous_trading_ai\execution`
- T7 execution monitoring slice is now partly complete: `ui_api.adapters._build_open_trade_drilldown()` annotates open positions with manual_user origin/exclusion metadata plus separate manual floating/open-trade counts, and `ui-front/src/app/execution/page.tsx` now surfaces a dedicated manual monitoring bucket, origin filter, and explicit manual-vs-autonomous badges across the open-trade ledgers
- Verified the T7 execution-monitoring slice with `python -m pytest autonomous_trading_ai/tests/test_manual_trade_identity_ui_api.py`, `python -m compileall autonomous_trading_ai\ui_api`, and a production `npm run build` in `ui-front`
- T7 audit-timeline visibility is now complete: `ui_api.adapters.load_audit_timeline()` annotates manual_user rows with explicit lifecycle stages (preview intent, submit intent, broker result, reconciliation gap, trade-log activity), and `ui-front/src/app/audit/page.tsx` now adds origin/manual-stage filters plus operator summary cards so manual-ticket evidence can be isolated from autonomous audit flow
- Verified the T7 audit-timeline slice with `python -m pytest autonomous_trading_ai/tests/test_manual_trade_identity_ui_api.py autonomous_trading_ai/tests/test_manual_trade_risk_api.py`, `python -m compileall autonomous_trading_ai\ui_api`, and a production `npm run build` in `ui-front`
- T7 lifecycle depth is now complete: `ui_api.adapters.load_execution_summary()` now emits a manual-ticket lifecycle dossier that correlates preview/submit/broker-result audit rows with manual open positions, trade-context journal hits, recent manual trade-log rows, and unmatched-close reconciliation gaps, while `ui-front/src/app/execution/page.tsx` surfaces those per-ticket runtime-linkage states in a dedicated reconciliation section
- Verified the T7 lifecycle-depth slice with `python -m pytest autonomous_trading_ai/tests/test_manual_trade_identity_ui_api.py autonomous_trading_ai/tests/test_manual_trade_risk_api.py`, `python -m compileall autonomous_trading_ai\ui_api autonomous_trading_ai\execution`, and a production `npm run build` in `ui-front`
- T8 symbol/account-read hardening is now partly complete: `ui_api/app.py` surfaces resolver context for symbol-spec fetch failures plus distinct account snapshot vs missing-equity validation codes, `ui-front/src/app/manual-ticket/page.tsx` now gives operator guidance for symbol metadata and `% equity` account-snapshot failures, and `tests/test_manual_trade_risk_api.py` covers those edge cases explicitly
- Verified the T8 symbol/account-read hardening slice with `python -m pytest autonomous_trading_ai/tests/test_manual_trade_risk_api.py`, `python -m compileall autonomous_trading_ai/ui_api autonomous_trading_ai/execution`, and a production `npm run build` in `ui-front`
- Next highest-value slice after this is finishing the remaining unchecked T8 item for missing MT5/account-info handling around broker validation/live submit paths, or exposing the live submit UI with explicit operator gating once that hardening is complete
