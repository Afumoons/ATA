...
- Verified the latest T4 segregation slice with `python -m pytest autonomous_trading_ai/tests/test_manual_trade_identity_ui_api.py autonomous_trading_ai/tests/test_manual_trade_risk_api.py autonomous_trading_ai/tests/test_strategy_live_stats_manual_buckets.py` and a production `npm run build` in `ui-front`
- Next highest-value slice after this is the remaining T4 research-attribution hardening plus audit-trail creation/execution entries before any live submit endpoint lands
