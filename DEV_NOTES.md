...
- Next highest-value slice after this is finishing the remaining T4 gap for broker-side manual magic/execution-origin tagging if MT5 order-submit plumbing supports it cleanly, otherwise start T5 with the explicit preview card and confirmation gate while reusing the new audit helpers
