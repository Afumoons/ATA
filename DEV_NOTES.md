...
- T4 manual identity / segregation is now partially complete: `execution/manual_trade_identity.py` centralizes the manual marker contract (`manual_user` / `operator_ui` / `is_manual` / `exclude_from_strategy_eval` / `clio-manual-user`), the risk-calc preview now sources those tags from one helper, and `ui_api.adapters.load_strategy_live_stats_snapshot()` excludes `manual_*` buckets from autonomous totals/top-active summaries while surfacing them separately for manual-only diagnostics
- Verified the T4 identity/segregation slice with `python -m pytest autonomous_trading_ai/tests/test_manual_trade_identity_ui_api.py autonomous_trading_ai/tests/test_manual_trade_risk_api.py autonomous_trading_ai/tests/test_strategy_live_stats_manual_buckets.py` and `python -m compileall autonomous_trading_ai/ui_api autonomous_trading_ai/execution`
- Next highest-value slice after this is the remaining T4 segregation work: keep manual_user trades out of research attribution and make manual/autonomous distinction explicit in drift/audit views before any live submit endpoint lands
