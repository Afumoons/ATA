...
- T8 symbol/account-read hardening is now complete: `ui_api/app.py` now distinguishes symbol-spec fetch failures, account snapshot vs missing-equity calculator failures, MT5 terminal/account/symbol availability failures during broker validation, and MT5 transport failures during live submit with operator-readable codes; `execution/manual_trade_broker_validation.py` and `execution/manual_trade_submit.py` now attach MT5 context/failure classification so preview/submit failures stay explicit and audit-friendly
...
- Next highest-value slice after this is exposing the live submit UI with explicit operator gating, while keeping manual_user segregation and audit requirements intact
